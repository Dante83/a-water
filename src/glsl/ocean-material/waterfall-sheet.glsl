...
//(FLOW_FADE_MIN_M 3 cm → FLOW_FADE_FULL_M 10 cm of a-land's baked depth, guarded by 0.5-3 cm of
//its level over the G-buffer ground under the pixel) and, inside a fall's corridor, stepping aside where its level is steep
...
float creekVisibleAt(vec2 xz, float groundY, bool haveGround){
  ...
  //Since 2026-09-22: the 3-10 cm fade runs on the baked depth (RT0.g), and the rendered
  //ground only guards 0.5-3 cm, as on the flowing surface (it stopped blinking with the view).
  vec2 uvD = (xz - waterFieldCascadeCenter[0]) / (2.0 * waterFieldCascadeHalfWidth[0]) + 0.5;
  float vis = smoothstep(0.03, 0.10, texture2D(waterFieldCascade0, uvD).g);
  if(haveGround){
    //descending view rays only, as on the flowing surface (a rising ray reads the ground behind)
    float rayDown = -normalize(vec3(xz.x, level, xz.y) - cameraPosition).y;
    vis *= mix(1.0, smoothstep(0.005, 0.03, level - groundY), smoothstep(0.03, 0.12, rayDown));
  }
