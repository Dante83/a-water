...
//already duplicates cascadeDisplacementArray/cascadePatchSizes/
...
void main() {
  ...
  //The weight test looks at the vertex AND its neighbours 2 m away (the coarser ring's
  //cell): a still-water vertex right beside the creek has weight 0, and dropping it threw
  //away the triangles joining it to the creek, while the still surface was dithering its
  //share of that same rim away, so neither drew (browser round 5, 2026-09-14: a bare band
  //between every creek and the pond it runs into).
  const float CULL_INLAND = 3.0;
  const float CULL_DROP = 30.0;
  const float CULL_REACH = 2.0;
  float cullW = flowHandoffFieldWeightAt(worldXZ)
              + flowHandoffFieldWeightAt(worldXZ + vec2(CULL_REACH, 0.0))
              + flowHandoffFieldWeightAt(worldXZ - vec2(CULL_REACH, 0.0))
              + flowHandoffFieldWeightAt(worldXZ + vec2(0.0, CULL_REACH))
              + flowHandoffFieldWeightAt(worldXZ - vec2(0.0, CULL_REACH));
  if(cullW <= 0.0 || field.b < -CULL_INLAND){
