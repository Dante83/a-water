...
void main(){
  ...
  vec2 worldPixel = onePixel * halfWidthOfPatchOverWaveScaleFactor;
  ...
  vec3 v00 = vec3(-worldPixel.x, 0.0, worldPixel.y) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;
  texOffset = texture2D(waveHeightTexture, uv + vec2(0.0, onePixel.y)).xyz;
  vec3 v10 = vec3(0.0, 0.0, worldPixel.y) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;
  texOffset = texture2D(waveHeightTexture, uv + vec2(onePixel.x, onePixel.y)).xyz;
  vec3 v20 = vec3(worldPixel.x, 0.0, worldPixel.y) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;

  texOffset = texture2D(waveHeightTexture, uv + vec2(-onePixel.x, 0.0)).xyz;
  vec3 v01 = vec3(-worldPixel.x, 0.0, 0.0) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;
  texOffset = texture2D(waveHeightTexture, uv + vec2(onePixel.x, 0.0)).xyz;
  vec3 v11 = vec3(worldPixel.x, 0.0, 0.0) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;

  texOffset = texture2D(waveHeightTexture, uv + vec2(-onePixel.x, -onePixel.y)).xyz;
  vec3 v02 = vec3(-worldPixel.x, 0.0, -worldPixel.y) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;
  texOffset = texture2D(waveHeightTexture, uv + vec2(0.0, -onePixel.y)).xyz;
  vec3 v12 = vec3(0.0, 0.0, -worldPixel.y) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;
  texOffset = texture2D(waveHeightTexture, uv + vec2(onePixel.x, -onePixel.y)).xyz;
  vec3 v22 = vec3(worldPixel.x, 0.0, -worldPixel.y) + vec3(-texOffset.x, texOffset.y, -texOffset.z) - s01;
