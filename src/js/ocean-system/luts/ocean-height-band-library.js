AWater.AOcean.LUTlibraries.OceanHeightBandLibrary = function(parentOceanGrid){
  ...
  this.hkVar = hkRenderer.addVariable('textureHk', materials.hkShaderMaterialData.fragmentShader, this.hkTexture);
  let hkVar = this.hkVar;
  this.hkVar.minFilter = THREE.ClosestFilter;
  this.hkVar.magFilter = THREE.ClosestFilter;
  hkRenderer.setVariableDependencies(hkVar, []);//Note: We use manual texture dependency injection here.
  hkVar.material.uniforms = JSON.parse(JSON.stringify(materials.hkShaderMaterialData.uniforms));
  ...
  for(let i = 0; i < this.numLevels; i++){
    ...
    this.hkBandVars.push(hkRenderer.addVariable(`textureHkBand_${i}`, materials.amplitudeFilterShaderMaterial.fragmentShader(), this.hkBandTextures[i]));
    hkRenderer.setVariableDependencies(this.hkBandVars[i], [hkVar]);//Note: We use manual texture dependency injection here.
    this.hkBandVars[i].material.uniforms = JSON.parse(JSON.stringify(materials.amplitudeFilterShaderMaterial.uniforms));
    ...
    this.hkBandVars[i].minFilter = THREE.ClosestFilter;
    this.hkBandVars[i].magFilter = THREE.ClosestFilter;
  }
